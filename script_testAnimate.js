...
async function fetchMidrangeForecast(city = '臺中市') {
  const url = `https://opendata.cwa.gov.tw/api/v1/rest/datastore/F-C0032-001`
            + `?Authorization=CWA-F5269C99-76E7-4501-9A20-73782145C9CE`
            + `&format=JSON&locationName=${encodeURIComponent(city)}`
            + `&elementName=Wx`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`CWB 中期預報錯誤：${res.status}`);
  const j = await res.json();
  return j.records.locations[0].location[0].weatherElement[0].time
          .map(t => t.parameter.parameterName);
}

// --- 2. 初始化：先拉出來，然後在 startApp 呼 ---
async function initWeatherIntegration() {
  try {
    const live = await fetchRealtimeWeather();       // 取當前「晴、雲、雨…」
    const forecast = await fetchMidrangeForecast();  // 取未來兩日「Wx」清單

    // 如果預報裡有「雨」，把 select 改成「陰有雨」或「陰有雷雨」
    if (forecast.some(p => p.includes('雨'))) {
      weatherSelect.value = live.includes('雷') ? '陰有雷雨' : '陰有雨';
    } else {
      weatherSelect.value = live;
    }

  } catch (e) {
    console.error('天氣 API 取得失敗：', e);
    weatherSelect.value = '無法取得天氣資料';
  }
  updateBackground();
}
function startApp() {
...
// 1-1. 取即時觀測（StationName 可改成任何你想要的氣象站）
async function fetchRealtimeWeather() {
  const station = encodeURIComponent('臺中'); // 或其他站名
  const url = `https://opendata.cwa.gov.tw/api/v1/rest/datastore/O-A0003-001` +
              `?Authorization=CWA-F5269C99-76E7-4501-9A20-73782145C9CE` +
              `&format=JSON&StationName=${station}&WeatherElement=Weather`;
  const res = await fetch(url);
  if (!res.ok) throw new Error(`CWB 即時觀測錯誤：${res.status}`);
  const json = await res.json();
  // 取第一筆觀測值的 Weather 文字 (如「晴」／「多雲」)
  return json.records.location[0].weatherElement[0].time[0].elementValue.value;
}
